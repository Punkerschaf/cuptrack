...
function CreateUserDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.type !== 'drinker') delete payload.pin;
      await api.createUser(payload);
      onCreated();
      onClose();
      setForm({ username: '', displayName: '', password: '', type: 'drinker', pin: generatePin() });
    } catch (e: unknown) {
      ...
      <DialogContent>
        ...
        <TextField
          ...
          onChange={(e) => {
            ...
            setForm({ ...form, type: newType, pin: newType === 'drinker' ? generatePin() : '' });
          }}
          margin="dense"
        >
          <MenuItem value="admin">{t('users.typeAdmin')}</MenuItem>
        ...
        {form.type === 'drinker' && (
          <TextField
            ...
            error={form.pin.length > 0 && form.pin.length < 4}
            helperText={form.pin.length > 0 && form.pin.length < 4 ? t('users.pinMustBe4Digits') : t('users.pinAutoGenerated')}
          />
        )}
      </DialogContent>
      <DialogActions>
